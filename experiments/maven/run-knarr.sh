...
RES=$RESULTS_DIR/maven-knarr

JQF_SENSITIVITY=3
JQF_DEBUG="-Xdebug -Xrunjdwp:server=y,transport=dt_socket,address=5555,suspend=n"
JQF_OPTS="-Xmx3G"
JQF_CMD="JVM_OPTS=\"-DcentralPort=$PORT -Dtime=$(($DURATION + 60)) -Djqf.ei.UNIQUE_SENSITIVITY=$JQF_SENSITIVITY $JQF_OPTS $JQF_DEBUG\" $JQF_DIR/bin/jqf-ei -c $JQF_DIR/examples/target/classes:$JQF_DIR/examples/target/test-classes:`$JQF_DIR/scripts/classpath.sh`:`$MAVEN_DIR/classpath.sh` edu.berkeley.cs.jqf.examples.maven.ModelReaderTest testWithGenerator $RES"

Z3_OUTPUT_DIR="-DZ3_OUTPUT_DIR=/home/lganchin/repos/swe/jqf-artifact/results/maven-knarr/"
CENTRAL_DEBUG="-Xdebug -Xrunjdwp:server=y,transport=dt_socket,address=5554,suspend=n"
CENTRAL_OPTS="-Xmx3G -Xss1G"
CENTRAL_CMD="LD_LIBRARY_PATH=$Z3_DIR $JAVA_HOME/bin/java $CENTRAL_DEBUG $CENTRAL_OPTS -Djava.library.path=$Z3_DIR $Z3_OUTPUT_DIR -cp $GREEN_DIR/green/green.jar:`$JQF_DIR/scripts/classpath.sh` -DcentralPort=$PORT edu.berkeley.cs.jqf.fuzz.central.Central $ROOT/experiments/maven/properties"

#KNARR_MAX_CS="-DMAX_CONSTRAINTS=1000"
KNARR_DEBUG="-Xdebug -Xrunjdwp:server=y,transport=dt_socket,address=5556,suspend=n"
KNARR_OPTS="-Xmx20G -Xss1G"
KNARR_CMD="JVM_OPTS=\"$KNARR_MAX_CS -DcentralPort=$PORT -Dsize=15 $KNARR_OPTS $KNARR_DEBUG\" $JQF_DIR/bin/jqf-knarr -c $JQF_DIR/examples/target/classes-inst:$JQF_DIR/examples/target/test-classes-inst:`$MAVEN_DIR-inst/classpath.sh`:`$JQF_DIR/scripts/classpath.sh` edu.berkeley.cs.jqf.examples.maven.ModelReaderTest testWithGenerator"

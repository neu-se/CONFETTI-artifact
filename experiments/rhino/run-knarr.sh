...
CENTRAL_CMD="LD_LIBRARY_PATH=$Z3_DIR $JAVA_HOME/bin/java $CENTRAL_DEBUG $CENTRAL_OPTS -Djava.library.path=$Z3_DIR $Z3_OUTPUT_DIR -cp $GREEN_DIR/green/green.jar:`$JQF_DIR/scripts/classpath.sh` -DcentralPort=$PORT edu.berkeley.cs.jqf.fuzz.central.Central $ROOT/experiments/rhino/properties"
...
KNARR_CMD="JVM_OPTS=\"$KNARR_MAX_CS -DcentralPort=$PORT -Dsize=15 $KNARR_OPTS $KNARR_DEBUG\" $JQF_DIR/bin/jqf-knarr -c $JQF_DIR/examples/target/classes-inst:$JQF_DIR/examples/target/test-classes-inst:`$RHINO_DIR-inst/classpath.sh`:`$JQF_DIR/scripts/classpath.sh` edu.berkeley.cs.jqf.examples.rhino.CompilerTest testWithGenerator"
